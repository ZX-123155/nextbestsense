...
class Trainer:
    ...
    def train(self) -> None:
        ...
        # print final image-quality metrics summary
        if self.pipeline.datamanager.eval_dataset:
            try:
                final_metrics = self.pipeline.get_average_eval_image_metrics(step=step)
                summary_table = Table(
                    title=None,
                    show_header=False,
                    box=box.MINIMAL,
                ...
                if "psnr_roi" in final_metrics:
                    summary_table.add_row("Final PSNR (ROI)", f"{float(final_metrics['psnr_roi']):.4f}")
                if "ssim_roi" in final_metrics:
                    summary_table.add_row("Final SSIM (ROI)", f"{float(final_metrics['ssim_roi']):.4f}")
                if "lpips_roi" in final_metrics:
                    summary_table.add_row("Final LPIPS (ROI)", f"{float(final_metrics['lpips_roi']):.4f}")
                if "psnr_full" in final_metrics:
                    summary_table.add_row("Final PSNR (Full)", f"{float(final_metrics['psnr_full']):.4f}")
                if "ssim_full" in final_metrics:
                    summary_table.add_row("Final SSIM (Full)", f"{float(final_metrics['ssim_full']):.4f}")
                if "lpips_full" in final_metrics:
                    summary_table.add_row("Final LPIPS (Full)", f"{float(final_metrics['lpips_full']):.4f}")

                pipeline_ref = self.pipeline.module if hasattr(self.pipeline, "module") else self.pipeline
                nbv_ig_history = getattr(pipeline_ref, "nbv_ig_history", [])
                if nbv_ig_history:
                    for round_item in nbv_ig_history:
                        if round_item.get("is_touch", False):
                            continue
                        round_id = int(round_item.get("round", 0))
                        views = round_item.get("views", [])
                        for view_item in views:
                            view_idx = int(view_item.get("view_idx", -1))
                            selected = bool(view_item.get("selected", False))
                            label = f"NBV{round_id} View{view_idx}{'*' if selected else ''}"
                            ig_roi = float(view_item.get("ig_roi", 0.0))
                            ig_bg = float(view_item.get("ig_bg", 0.0))
                            r_ratio = float(view_item.get("r", 0.0))
                            score = view_item.get("score", None)
                            if score is None:
                                value = f"IG_roi={ig_roi:.4e}, IG_bg={ig_bg:.4e}, R={r_ratio:.4f}"
                            else:
                                value = f"IG_roi={ig_roi:.4e}, IG_bg={ig_bg:.4e}, R={r_ratio:.4f}, S={float(score):.6f}"
                            summary_table.add_row(label, value)
                if summary_table.row_count > 0:
                    CONSOLE.print(
                        Panel(summary_table, title="[bold][cyan]Final Metrics[/bold][cyan]", expand=False)
                    )
            except Exception as exc:
                CONSOLE.log(f"Could not compute final eval metrics summary: {exc}")
